...
main() {
    local os=$(get_os)
    local arch=$(get_arch)
    local dest_file="${downloadFolder}/ip_${os}_${arch}.tar.gz"

    if [[ $# -eq 0 ]]; then
        asset_path=$(
            command curl -sSf https://github.com/axetroy/ip-cli/releases |
                command grep -o "/axetroy/ip-cli/releases/download/.*/ip_${os}_${arch}\\.tar.gz" |
                command head -n 1
        )
        if [[ ! "$asset_path" ]]; then exit 1; fi
        asset_uri="https://github.com${asset_path}"
    else
        asset_uri="https://github.com/axetroy/ip-cli/releases/download/${1}/ip_${os}_${arch}\\.tar.gz"
    fi
